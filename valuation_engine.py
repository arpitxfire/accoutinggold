...
def run_valuation(ticker: str) -> dict:
    ...
    selector_inputs = {
        ...
        "stable_growth":        fd.get("stable_growth",
                                fd.get("inflation_rate", 0.03) + fd.get("real_growth_rate", 0.02) * 0.5),
        "inflation_rate":       fd.get("inflation_rate",  0.03),
        "real_growth_rate":     fd.get("real_growth_rate", 0.02),
        "risk_free_rate":       fd.get("risk_free_rate",  0.05),
        "erp":                  fd.get("erp",             0.055),
        "beta":                 fd.get("beta",            1.0),
        "wacc":                 fd.get("wacc",            0.10),
        "cost_of_equity":       fd.get("cost_of_equity",  0.12),
        "cyclical_negative":    fd.get("cyclical_negative",    False),
        "temporary_negative":   fd.get("temporary_negative",   False),
        "excess_debt_negative": fd.get("excess_debt_negative", False),
        "bankruptcy_likely":    fd.get("bankruptcy_likely",    False),
    ...
    dps = fd.get("dividends_total", 0) / shares if shares > 0 else 0
    eps = fd.get("net_income", 0) / shares if shares > 0 else 0

    fcfe_total = compute_fcfe(
        fd.get("net_income", 0), fd.get("depreciation", 0), fd.get("capex", 0),
        fd.get("delta_wc", 0), fd.get("debt_ratio", 0.3)
    )
    fcfe_ps = fcfe_total / shares if shares > 0 else 0

    fcff_total = compute_fcff(
        fd.get("ebit", 0), fd.get("tax_rate", 0.25), fd.get("depreciation", 0),
        fd.get("capex", 0), fd.get("delta_wc", 0)
    )

    ke   = fd.get("cost_of_equity", 0.12)
    wacc = fd.get("wacc", 0.10)
    hg   = fd.get("firm_growth_rate", 0.08)
    sg   = fd.get("stable_growth",
                  fd.get("inflation_rate", 0.03) + fd.get("real_growth_rate", 0.02) * 0.5)
