...
# ─────────────────────────────────────────────────────────────────────────────
# SOURCE 3: Stooq (free, no key needed, good for Indian + US stocks)
# ─────────────────────────────────────────────────────────────────────────────
def _fetch_stooq(ticker, years=3):
    """
    Fallback: Stooq historical data. No API key needed.
    Handles both US (AAPL → aapl.us) and Indian (.NS → .in suffix).
    """
    import requests

    # Map ticker to Stooq format
    if ticker.endswith(".NS") or ticker.endswith(".BO"):
        # Indian NSE/BSE stocks — Stooq uses .in suffix
        stooq_sym = ticker.replace(".NS", ".in").replace(".BO", ".in").lower()
    elif "." in ticker and len(ticker.split(".")[-1]) == 2:
        # European (e.g. P911.DE → p911.de)
    ...
    if "Close" not in df.columns or len(df) < 20:
        raise ValueError(f"Stooq: insufficient data for {stooq_sym}")
...
# ─────────────────────────────────────────────────────────────────────────────
# SOURCE 4: Alpha Vantage (free key)
# ─────────────────────────────────────────────────────────────────────────────
...
def get_stock_data(ticker, period="3y"):
    ...
    for name, fetch_fn in _SOURCES:
        try:
            ...
            elif name in ("Yahoo Direct CSV", "Stooq"):
